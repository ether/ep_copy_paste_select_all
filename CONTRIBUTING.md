# Contributing to ep_copy_paste_select_all

Thanks for helping improve `ep_copy_paste_select_all` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_copy_paste_select_all` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_copy_paste_select_all from your local clone
pnpm run plugins i --path ../ep_copy_paste_select_all

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_copy_paste_select_all/` are picked up after a server restart.
...
* Etherpad version, ep_copy_paste_select_all version, browser + OS
* Steps to reproduce
